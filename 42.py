...
def point_in_triangle(point, triangle) -> bool:
    ...
    for i in range(len(triangle[0])):
        if ((triangle[1][i] <= point[1] < triangle[1][i - 1]) or (triangle[1][i - 1] <= point[1] < triangle[1][i])) and (point[0] > (triangle[0][i - 1] - triangle[0][i]) * (point[1] - triangle[1][i]) / (triangle[1][i - 1] - triangle[1][i]) + triangle[0][i]):
            result = 1 - result
